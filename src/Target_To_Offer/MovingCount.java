 ...
 * 请问该机器人能够达到多少个格子？
 *
 * PS：问的是能到达多少个格子，没说不能走已经走过的格子。所以不用回溯，直接深搜。
 */
public class MovingCount {
    ...
    public class Solution {
        private final  int[][] next = {{0, -1}, {0, 1}, {-1, 0}, {1, 0}};   //上下左右的坐标变化
        private int count = 0;  //记录到达的格子数
        private int rows;       //不用麻烦每次多传这俩参数了
        private int cols;       //同上
        private int[][] digitSum;//存储每个格子的坐标数位和
        public int movingCount(int threshold, int rows, int cols) {
            if (rows <= 0 || cols <= 0 || threshold < 0)
            ...
            stack.push(0);
            marked[0][0] = true;
            int cur, r, c;            //将矩阵变为一维数组后的索引！！！！和矩阵每个格子的坐标
            while (!stack.isEmpty()){
                cur = stack.pop();
                count++;
                for(int[] nextStep : next){
                    r = cur / cols + nextStep[0];
                    c = cur % cols + nextStep[1];
                    if(digitSum[r][c] <= threshold && r >= 0 && r < rows && c >= 0 && c < cols && !marked[r][c]) {
                        stack.push(c + r * cols);
                        marked[r][c] = true;
                    }
                }
